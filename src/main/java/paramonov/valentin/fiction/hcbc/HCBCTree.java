...
public class HCBCTree extends QuadTree<HCBCTree, HCBCBlock> {
    ...
    protected int findPlace(HCBCBlock block) {
        ...
        int horizontalQuad = quad(element.getX(), element.getWidth(), block.getX(), block.getWidth());

        if(horizontalQuad < 0) {
            return -1;
        }

        int verticalQuad = quad(element.getY(), element.getHeight(), block.getY(), block.getHeight());
    ...
    int quad(int regionStart, int regionSize, int blockStart, int blockSize) {
        ...
        if(belongs(regionStart, regionMid, blockStart)) {
            if(blockSize > halfSize) {
                return -1;
            }
            return 0;
        } else if(belongs(regionMid, regionEnd, blockStart)) {
            if(blockSize > regionSize / 2) {
                return -1;
            }
            return 1;
        }
